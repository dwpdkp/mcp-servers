...
| **altaro-mcp** | Read-only Altaro VM Backup per-VM results, failure streaks, in-progress backups and service state on STEAMHV1/STEAMHV2, read from the Windows event log over SSH | In-house | No mutating tools; VM name filter validated by regex; PowerShell sent via `-EncodedCommand` |
