...
| **altaro-mcp** | Read-only Altaro VM Backup on STEAMHV1/STEAMHV2: per-VM results and failure streaks (event log), live job progress mapped to VMs and per-VM last/next/offsite status (Altaro REST API), service state; all over SSH | In-house | No mutating tools; dedicated AD account `svc-altaro-mcp` (enigma 22304) for the API, password passed over SSH stdin, never on a command line; VM name filter validated by regex |
...
| 2026-09-24 | Doug Pearson | Added altaro-mcp; tool-search descriptions and `instructions=` for bacularis-mcp and proxmox-mcp |
